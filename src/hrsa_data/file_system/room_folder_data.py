...
class RoomFolderData:
    ...
    def create_new_room_folder(self) -> FileSystemResultData:
        # Create Room Folder
        new_room_folder_path = pathlib.Path(self.room_folder_root_path)
        ...
        # Create Audio Folder
        self.room_audio_folder_data.audio_folder_root_path = os.path.join(self.room_folder_root_path, __hdfsc__.AUDIO_FOLDER_NAME)
        file_system_result_data: FileSystemResultData = self.room_audio_folder_data.create_new_audio_folder()
        if not file_system_result_data.is_success:
            return file_system_result_data
        # Create Dialogue Ink File
        self.dialogue_ink_file_path = os.path.join(self.room_folder_root_path, __hdfsc__.DIALOGUE_INK_FILE_NAME)
        ...
        # Dialogue Ink JSON File Path
        self.dialogue_ink_json_file_path = os.path.join(self.room_folder_root_path, __hdfsc__.DIALOGUE_INK_JSON_FILE_NAME)
